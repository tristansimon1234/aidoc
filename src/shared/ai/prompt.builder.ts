...
/**
 * Prompt for Gemini video analysis — extracts the key documentable steps from a
 * screen recording. Used both by the inline `analyzeVideoWithGemini` path
 * (short recordings, on Vercel) and, serialized into the request body, by the
 * `video-service` async path that compresses + chunks long recordings off
 * Vercel (see `video-service/server.js /process-and-analyze`). Keeping the
 * prompt authored here — and shipping it to the worker rather than duplicating
 * it — preserves the "all AI prompts live in prompt.builder.ts" rule even when
 * the model call physically runs in the external service.
 *
 * `segmentSeconds`, when provided, scales the "how many steps" guidance to the
 * length of the clip (or chunk) being analysed, so a 10-minute chunk asks for
 * more steps than a 90-second one. Omit it for the legacy short-video path,
 * which keeps the original "5-10 steps for a 1-3 minute video" wording.
 */
