...
- Available Remotion namespace (use as \`Remotion.foo\`):
  ...
  **Pre-built designed helpers (USE THESE — saves boilerplate, ensures consistency):**
  ...
  - \`<Remotion.Icons.Cpu size={14} color='currentColor' />\` — Lucide icons, pre-wrapped with a thin 1.5px stroke (Linear/Vercel weight). Available names (lucide-native naming accepted):
    Plug, Mic, Check, Message / MessageSquare, Search, Zap, Code, Settings, MousePointer, Send, Loader, Bell, User, Lock, Globe, ChevronRight, Plus, X, Copy, Play, Pause, Volume / Volume2, Image, ArrowRight, ArrowUpRight, Activity, Cpu, Layers, Database, GitBranch, FileText, Cloud, Workflow, Boxes, BarChart / BarChart2 / BarChart3, TrendingUp, TrendingDown, Star, Heart. They take \`size\` (px) and standard SVG props (stroke is already 1.5; override only if you really need a heavier weight). Anything not on this list will fail to render — pick the closest match (e.g. for analytics use BarChart3, for AI / processing use Cpu / Workflow, for status use Check / Activity).
