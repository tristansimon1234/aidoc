...
- "brand": the product's own colors, as seen on the screenshots, hex "#RRGGBB". "accent" = its main brand color (logo, primary buttons). "background" = the main background color of its interface (the page behind the content: often white or a very light grey; dark only if the product itself is dark). The video uses them so it looks like the product. Fill "accent2" with the same color as "accent" (a lighter tint is derived from it).
...
- Palette: the product's own colors. brand.background (the product's interface background, often white or light grey) for the canvas and brand.text for text on it; mockup cards like the real product (on a light canvas: white cards with a thin #0000001A border and a soft shadow; on a dark canvas: neutral cards), brand.accent for the key element, brand.accent2 for small highlights. No other colors, no rainbow gradients.
