...
// Toutes les icônes lucide sont accessibles via Icons[name] (le bundle Remotion est construit une
// fois par déploiement, son poids ne compte pas).
...
import type { Shot } from '../props.js'

/**
 * Composants prêts à l'emploi pour le code des scènes écrit par l'IA (namespace `Remotion`),
 * repris de l'ancienne plateforme. Ils évitent de réécrire à chaque scène un cadre de navigateur,
 * un curseur ou une lueur, sans imposer de style.
 */
...
interface PillProps {
  /** Pill label text. */
  children: React.ReactNode
  /** Color treatment. accent uses brand.accent — the Pill takes
  ...
  dot?: boolean
  /** When tone='accent', passes brand.accent through. */
  accentColor?: string
  style?: React.CSSProperties
}
...
export const AccentGlow: React.FC<AccentGlowProps> = ({
  ...
  const pulse = frame !== undefined ? 0.3 + 0.15 * Math.sin(frame / 14) : (opacity ?? 0.4)
...
interface ScreenshotProps {
  /** Une des captures reçues par la scène (`shots[i]`). */
  shot: Shot
  /**
   * Zone à mettre en valeur, en % de la capture (0-100) : la caméra zoome dessus pendant
   * [startFrame, startFrame + zoomFrames]. Sans `focus`, léger zoom lent sur le centre.
   */
  focus?: { x: number; y: number; zoom?: number }
  startFrame?: number
  zoomFrames?: number
  /** Coins arrondis en px. Défaut 16. */
  radius?: number
  style?: React.CSSProperties
}

/**
 * Vraie capture du produit, affichée en entier (object-fit: contain) dans son conteneur, avec un
 * mouvement de caméra (zoom vers `focus`). C'est la base la plus crédible pour montrer le produit.
 */
export const Screenshot: React.FC<ScreenshotProps> = ({
  ...
  const span = zoomFrames ?? Math.max(1, durationInFrames - startFrame)
  const t = interpolate(frame, [startFrame, startFrame + span], [0, 1], {
    extrapolateLeft: 'clamp',
    extrapolateRight: 'clamp',
  })
  const ease = 1 - Math.pow(1 - t, 3)
  const zoom = 1 + ((focus?.zoom ?? 1.08) - 1) * ease
