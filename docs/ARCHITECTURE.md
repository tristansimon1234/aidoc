...
### Token-based Billing
Every metered op (`doc_run`, `voiceover`, `try_doc`, `chat_sessions`, `marketing_video`) is incremented in `usage_counters` after success. Token weight, real € COGS, and overage rates live in `src/features/billing/billing.service.ts` as constants (`TOKEN_COSTS`, `EURO_COSTS`, `OVERAGE_EUR`, `OVERAGE_ENABLED_PLANS`) — tunable in code, no migration. Users see one percent (single bar in `/account?tab=billing`); admins see real € COGS and billable overage in `/admin/usage`.

### Quota Enforcement
`enforceQuotaOrThrow(teamId)` in `src/shared/middleware/quota.middleware.ts` runs before every metered op (3 chat routes + doc-gen / voiceover / try-doc / marketing-video). It calls `checkQuota()` (`billing.service.ts`) which sums `counters × TOKEN_COSTS` against the plan's `monthlyTokens`. Hard-cap plans (Free / Founder) that hit 100 % get a 402 `QUOTA_EXCEEDED`; overage-enabled plans (Team / Agency) always pass through — they'll be billed via `OVERAGE_EUR` once Stripe is live. The frontend shows a clear upgrade CTA on 402.
